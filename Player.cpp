...
{
    float speed;
    bool canSprint = sprinting && !isCrouching && !isExhausted && stamina > 0.0f;
...
void Player::updateStamina(float deltaTime, bool wantsSprint, bool isMoving)
{
    ...
    if (softBreathTimer > 0.0f) softBreathTimer -= deltaTime;

    bool canSprint = stamina > 0.0f && !isExhausted && !isCrouching;
    bool actuallySprinting = wantsSprint && isMoving && canSprint;
