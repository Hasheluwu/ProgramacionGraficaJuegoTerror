...
unsigned int TextureFromFile(const char* path, const std::string& directory)
{
    ...
    // Si falla, intentar solo con el nombre del archivo en el directorio local
    if (textureID == 0)
    {
        size_t lastSlash = texturePath.find_last_of("/\\");
        std::string justFilename = (lastSlash != std::string::npos)
            ? texturePath.substr(lastSlash + 1)
            : texturePath;

        std::string fallbackPath = directory + "/" + justFilename;

        textureID = SOIL_load_OGL_texture(
            fallbackPath.c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y);
        ...
        std::cout << "TEXTURA CARGADA (fallback): " << fallbackPath << std::endl;
    }
    else {
