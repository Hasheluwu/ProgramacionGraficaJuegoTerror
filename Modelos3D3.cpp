...
const glm::vec3 PLAYER_SPAWN(-56.0f, 2.0f, -86.807f);
...
// Cambiada de P a K para evitar conflicto con pausa
...
const float PURSUIT_SPEED = 5.0f;
...
{
    if (gamePaused) return;   // No mover cámara en pausa
...
int main()
{
    ...
    else
    {
        ...
        audio.SetVolume("musica_fondo_1", 0.95f);
        audio.SetVolume("musica_fondo_2", 0.95f);

        audio.LoadSound("musica_escape", "Resources/Audio/musica_escape.mp3", true);
        audio.SetVolume("musica_escape", 0.7f);
        ...
        audio.SetVolume("monstruo_alerta", 1.0f);
        ...
        audio.SetVolume("risa_fondo_extra", 0.7f);
        audio.SetVolume("corrida_fondo", 0.5f);
    ...
    // ==================================================
    // MENÚ PRINCIPAL (con nuevo sistema de acciones)
    // ==================================================
    ...
    while (!glfwWindowShouldClose(window) && menu.state != MenuState::PLAYING)
    {
        ...
        if (menu.state == MenuState::LOADING) {
            ...
            if (menu.loadingProgress >= 1.0f) menu.loadingProgress = 1.0f;
        }
    ...
    // ==================================================
    // DECLARACIONES DE SISTEMAS (SIN INICIALIZAR)
    // ==================================================
    ...
    // Coordenadas de la puerta de salida
    glm::vec3 salida_hinge1(143.0f, 0.0f, -115.0f);
    glm::vec3 salida_hinge2(143.0f, 0.0f, -116.5f);
    glm::vec3 salida_interact(142.0f, 1.6f, -115.75f);
    ...
    std::vector<PosicionRopero> ubicacionesRoperos = {
        ...
        { glm::vec3(13.95f,  -0.2f, -46.06f),   90.0f  },
        { glm::vec3(37.71f,  -0.2f, 10.47f),    90.0f  },
        { glm::vec3(-32.05f, -0.2f, 13.98f),    270.0f },
        { glm::vec3(-11.98f, -0.2f, -109.99f),  180.0f },
        { glm::vec3(-74.08f, -0.2f, -86.45f),   270.0f },
        { glm::vec3(40.43f,  -0.2f, -71.16f),   90.0f  },
        { glm::vec3(44.65f,  -0.2f, -101.26f),  0.0f   }
    };
    ...
    glm::vec3 key2Position = BlenderToOpenGL(31.657f, -22.245f, 2.227f);
    glm::vec3 key3Position = BlenderToOpenGL(-18.547f, -21.498f, 1.031f);
    ...
    // ==================================================
    // LAMBDA: IniciarJuego — hard reset completo
    // ==================================================
    auto IniciarJuego = [&]()
        {
            std::cout << "\n[RESET] Iniciando hard reset completo...\n" << std::endl;
            ...
            // --- Puertas normales + puerta de salida ---
            doors.clear();
            doorAABBMin.clear();
            doorAABBMax.clear();
            ...
            salidaOpenState = false;

            auto puerta_salida1_r = std::make_unique<Door>(
            ...
            auto puerta_salida2_r = std::make_unique<Door>(
                "Resources/Models/Casa/puerta_salida2.obj",
                salida_hinge2, salida_interact,
                0.0f, 90.0f, 120.0f, 4.0f, false, 1.2f,
                &puzzleCompleted, &salidaOpenState
            );
            doors.push_back(std::move(puerta_salida1_r));
            doors.push_back(std::move(puerta_salida2_r));
            ...
            // --- Mapa celda -> puerta (valor 4 en la matriz) ---
            celdaAPuertaIdx.clear();
            ...
            for (const auto& ubi : ubicacionesRoperos) {
                ...
                rd.puertaIzq = std::make_unique<Door>(
                    ...
                    r_local_hingeIzq, worldInteract,
                ...
                rd.puertaDer = std::make_unique<Door>(
                    ...
                    r_local_hingeDer, glm::vec3(0, -100, 0),
                    0.0f, 120.0f, 150.0f, 2.0f, false, 2.5f,
                    nullptr, rd.openState.get());
            ...
            keyPickedTimer = 0.0f;
            key2PickedTimer = 0.0f;
            key3PickedTimer = 0.0f;

            // --- Luces ---
            lightSystem = LightSystem();

            // --- Audio ambiental: detener todo y reiniciar timers ---
            ...
            // --- Replanificar primera ruta del monstruo ---
            PlanificarNuevaRuta();

            std::cout << "[RESET] Hard reset completado.\n" << std::endl;
        };
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (gamePaused) {
            ...
            // Renderizar escena en background (simplificado)
            glClearColor(0.01f, 0.01f, 0.02f, 1.0f);
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

            glm::mat4 view = player.camera.GetViewMatrix();
            glm::mat4 projection = glm::perspective(glm::radians(player.camera.Zoom),
                GetAspectRatio(), 0.01f, 1000.0f);
            glm::mat4 modelMat = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -1.0f, 0.0f));

            shader.use();
            // Pasar brillo del menú al shader
            glUniform1f(glGetUniformLocation(shader.ID, "brightness"), menu.brightness);
            glUniform1i(glGetUniformLocation(shader.ID, "flashlightOn"), false);
            ...
            // Establecer luces actuales (estáticas)
            for (int i = 0; i < NUM_LAMPS; i++) {
                ...
                if (i == 8) { pwr = lightSystem.intensities[i] * 1.4f; range = 10.0f; lin = 0.03f; quad = 0.008f; }
            ...
            model->Draw(shader.ID);   // solo la casa
            for (auto& d : doors) d->Draw(shader.ID, modelMat);
            ...
            menu.Render();   // overlay de pausa
        }
        else {
            // ========== JUEGO ACTIVO (sin pausa) ==========
            // Música
            ...
            // Detección del jugador por el monstruo
                ...
                if (!attackAnimDone) {
                    ...
                    if (monsterModel.animTime >= (float)totalTime - 0.05f) {
                        ...
                        if (playerLives <= 0) {
                            ...
                            gamePaused = false;
                            glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
                            float menuLastFrame2 = (float)glfwGetTime();
                            while (!glfwWindowShouldClose(window) && menu.state != MenuState::PLAYING) {
                                float n2 = (float)glfwGetTime();
                                float d2 = n2 - menuLastFrame2;
                                menuLastFrame2 = n2;
                                glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
                            ...
                            if (menu.state == MenuState::PLAYING) {
                                glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
                                IniciarJuego();    // hard reset completo
                                firstMouse = true;
                            }
            ...
            // Debug posición jugador (tecla O)
            ...
            // --- DEBUG: tecla 0 fuerza la victoria directamente ---
            bool zeroPressed = glfwGetKey(window, GLFW_KEY_0) == GLFW_PRESS;
            if (zeroPressed) {
                std::cout << "[DEBUG] Forzando victoria con tecla 0." << std::endl;
            ...
            // DEBUG: forzar puzzle resuelto (tecla 9)
            bool ninePressed = glfwGetKey(window, GLFW_KEY_9) == GLFW_PRESS;
            if (ninePressed) {
                puzzleCompleted = true;
                salidaOpenState = true;
                std::cout << "[DEBUG] puzzleCompleted forzado a true con tecla 9." << std::endl;
            }
            ...
            // --- Victoria: el jugador abre la puerta de salida con el puzzle resuelto ---
            if (puzzleCompleted && ePressedOnce) {
                Door* puertaSalida1 = doors[doors.size() - 2].get();
                Door* puertaSalida2 = doors[doors.size() - 1].get();
                if (puertaSalida1->isBeingLookedAt || puertaSalida2->isBeingLookedAt) {
                    std::cout << "[VICTORIA] Jugador escapo por la puerta final." << std::endl;
                    menu.state = MenuState::WIN;
                    gamePaused = true;
                    glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
                }
            ...
            hudState.playerLives = playerLives;
            ...
            shader.use();
            // Brillo del menú no se usa en juego, pasamos 1.0 por si acaso
            glUniform1f(glGetUniformLocation(shader.ID, "brightness"), 1.0f);
            ...
            if (debugVisionActive) {
                ...
                for (int r = 0; r < NUM_RAYOS_DEBUG; r++) {
                    ...
                    glm::vec3 origen = glm::vec3(monsterPos.x, 1.5f, monsterPos.z);
        ...
        // ---- Procesar acciones del menú (comunes a pausa y juego) ----
        if (action == MenuAction::RESUME_GAME) {
            ...
            // Reinicio rápido: solo vidas + posiciones, NO objetos/puertas/llaves
            playerLives = 3;
            player = Player(PLAYER_SPAWN);
            player.camera.MouseSensitivity = menu.mouseSensitivity;
            monsterPos = glm::vec3(129.0f, MONSTER_HEIGHT, -98.0f);
