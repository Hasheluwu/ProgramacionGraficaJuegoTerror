...
void HUD::Render(const HUDState& s)
{
    ...
    // ========== MOSTRAR VIDAS ==========
    std::string livesText = "Vidas: " + std::to_string(s.playerLives);
    DrawText(livesText, 20.0f, 20.0f, 1.0f, glm::vec4(1.0f, 0.2f, 0.2f, 1.0f));
    // ==================================
    ...
    {
        std::string msg = s.switchIsOn ? "[E] Apagar" : "[E] Encender";
        float tw = (float)msg.size() * 10.0f;
        DrawText(msg, cx - tw * 0.5f, cy + 40.0f, 1.0f, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
    }
