...
// ==============================================================
// Shaders internos de texto (2D, sin iluminacion)
// ==============================================================
...
void HUD::Render(const HUDState& s)
{
    ...
    if (s.hasKey)
    {
        DrawText("[LLAVE]", 20.0f, (float)screenH - 40.0f, 1.0f,
            glm::vec4(1.0f, 0.85f, 0.1f, 1.0f));
    }

    if (s.hasKey2)
    {
        DrawText("[LLAVE 2]", 20.0f, (float)screenH - 70.0f, 1.0f,
            glm::vec4(0.3f, 0.7f, 1.0f, 1.0f));
    }

    if (s.hasKey3)
    {
        DrawText("[LLAVE 3]", 20.0f, (float)screenH - 100.0f, 1.0f,
            glm::vec4(0.8f, 0.2f, 0.8f, 1.0f));
    }

    // ---- Mensaje "Llave recogida" con fade-out ----
    if (s.showKeyPickedMsg)
    {
        // La opacidad desvanece en el ultimo segundo
        ...
        DrawText(msg,
            ...
            cy - 90.0f,
            1.0f,
            glm::vec4(1.0f, 0.85f, 0.1f, alpha));
        ...
        DrawText(sub,
            ...
            cy - 64.0f,
            0.85f,
            glm::vec4(1.0f, 1.0f, 1.0f, alpha * 0.8f));
    }

    // ---- Mensaje "Llave 2 recogida" con fade-out ----
    ...
    if (s.showKey3PickedMsg)
    {
        ...
        float tw = (float)msg.size() * 10.0f;
        DrawText(msg, cx - tw * 0.5f, cy - 210.0f, 1.0f,
            glm::vec4(0.8f, 0.2f, 0.8f, alpha));

        std::string sub = "Puede abrir otra puerta bloqueada";
        float sw = (float)sub.size() * 8.0f;
        DrawText(sub, cx - sw * 0.5f, cy - 184.0f, 0.85f,
            glm::vec4(1.0f, 1.0f, 1.0f, alpha * 0.8f));
    }

    // ---- Prompt de item recogible ----
    if (s.lookingAtItem)
    {
        std::string msg = "[E] Recoger";
        float tw = (float)msg.size() * 10.0f;
        DrawText(msg, cx - tw * 0.5f, cy + 40.0f, 1.0f,
            glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
    }

    // ---- Prompt de puerta ----
    if (s.lookingAtDoor && !s.doorIsOpen)
    {
        ...
        if (bloqueada)
        {
            ...
            DrawText(sub, cx - sw * 0.5f, cy + 64.0f, 0.9f,
                glm::vec4(color.r, color.g, color.b, 0.7f));
        }
        else
        {
...
// ==============================================================
// UpdateProjection — origen arriba-izquierda, Y hacia abajo
// ==============================================================
