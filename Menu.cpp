...
bool Menu::Update(GLFWwindow* window, float deltaTime) {
    ...
    if (state == MenuState::MAIN) {
        ...
        if (clickNow && !prevEnter && hoveredItem != -1) {
            prevEnter = true;
            if (selectedItem == 0) { state = MenuState::LOADING; loadingProgress = 0.0f; }
            else if (selectedItem == 1) { state = MenuState::SETTINGS; }
            else if (selectedItem == 2) { state = MenuState::CREDITS; }
            else if (selectedItem == 3) glfwSetWindowShouldClose(window, true);
            return false;
        }
    }
